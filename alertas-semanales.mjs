...
const EMAIL_ORIGEN = 'RADAR NL Tech <onboarding@resend.dev>';
...
const HORAS_VENTANA = 24 * 7; // Semana completa
...
async function obtenerNovedadesSubvenciones() {
  ...
  return subs
    .map(s => {
      ...
      return {
        tipo: 'Subvención', titulo: s.titulo, entidad: s.entidad_responsable,
        presupuesto: s.presupuesto_total, dias, score: ia?.puntuacion_ia ?? null,
        tags, url: s.url_original
      };
    })
...
async function obtenerNovedadesLicitaciones() {
  ...
  return data
    ...
    .map(l => {
      ...
      return {
        tipo: 'Licitación', titulo: l.titulo, entidad: l.organismo,
        presupuesto: l.presupuesto || l.valor_estimado, dias, score: l.score ?? null,
        tags, url: l.enlace_oficial
      };
    })
    .filter(it => it.dias !== null && it.dias >= DIAS_MIN_MARGEN && it.tags.length > 0);
}

// ============ RESUMEN DE PIPELINE DE LA SEMANA ============
async function obtenerResumenPipeline() {
  const desde = new Date(Date.now() - HORAS_VENTANA * 60 * 60 * 1000).toISOString();
  ...
  if (error || !data) return { presentadas: 0, ganadas: 0, perdidas: 0 };

  return {
    presentadas: data.filter(d => d.estado === 'presentada').length,
    ganadas: data.filter(d => d.estado === 'ganada').length,
    perdidas: data.filter(d => d.estado === 'perdida').length
  };
}

function generarHTML(items, pipeline) {
  const filas = items
    .sort((a, b) => (b.score ?? 0) - (a.score ?? 0))
    .slice(0, 20) // el resumen semanal puede acumular mucho, limitamos a las 20 mejores
    .map(it => {
  ...
  <div style="background:#07090F;padding:24px 12px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
    <div style="max-width:700px;margin:0 auto;background:#0D111C;border:1px solid #1C2334;border-radius:14px;overflow:hidden">
      ...
      <div style="background:linear-gradient(90deg,#0D111C,#111726);padding:24px 28px;border-bottom:1px solid #1C2334">
        <div style="font-size:22px;font-weight:700;color:#E4F300">📡 RADAR — Resumen semanal</div>
        <div style="font-size:12px;color:#565F76;margin-top:2px">NL Tech Esports · Semana del ${fechaHoy}</div>
      </div>
      ...
      <table style="width:100%;border-collapse:collapse;border-bottom:1px solid #1C2334">
        <tr>
          <td style="padding:20px 28px">
            ...
            <div style="font-size:11px;color:#565F76;text-transform:uppercase">Nuevas esta semana</div>
          </td>
          <td style="padding:20px 28px">
            <div style="font-size:24px;font-weight:700;color:#FF7A45;font-family:-apple-system,sans-serif">${urgentesCount}</div>
            <div style="font-size:11px;color:#565F76;text-transform:uppercase">Urgentes ahora</div>
          </td>
        </tr>
      </table>
      ...
      <table style="width:100%;border-collapse:collapse;border-bottom:1px solid #1C2334">
        <tr>
          <td style="padding:16px 28px">
            <div style="font-size:11px;color:#565F76;text-transform:uppercase;margin-bottom:6px">📊 Actividad del equipo esta semana</div>
            <div style="font-size:13px;color:#EDEFF3">
              <span style="color:#9098AC">${pipeline.presentadas} presentada${pipeline.presentadas !== 1 ? 's' : ''}</span>
              &nbsp;·&nbsp;
              <span style="color:#E4F300;font-weight:700">${pipeline.ganadas} ganada${pipeline.ganadas !== 1 ? 's' : ''}</span>
              &nbsp;·&nbsp;
              <span style="color:#FF6B6B">${pipeline.perdidas} perdida${pipeline.perdidas !== 1 ? 's' : ''}</span>
            </div>
          </td>
        </tr>
      </table>
      ...
      <div style="padding:8px 20px">
        ...
        ${items.length > 20 ? `<div style="padding:10px;text-align:center;font-size:11px;color:#565F76">Mostrando las 20 mejores de ${items.length} totales — ver el resto en RADAR</div>` : ''}
      </div>

      <div style="padding:18px 28px;border-top:1px solid #1C2334;text-align:center">
        <a href="https://subvenciones.nltech.es" style="display:inline-block;background:#E4F300;color:#07090F;font-weight:700;font-size:13px;padding:10px 20px;border-radius:8px;text-decoration:none;font-family:-apple-system,sans-serif">Ver todo en RADAR →</a>
      </div>

      <div style="padding:14px 28px;text-align:center">
        <div style="font-size:10px;color:#565F76;font-family:-apple-system,sans-serif">NL Tech Esports S.L. · Resumen semanal automático (lunes)</div>
      </div>
...
async function enviarEmail(items, pipeline) {
  const html = generarHTML(items, pipeline);
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: EMAIL_ORIGEN,
      to: EMAIL_DESTINO,
      subject: `📡 RADAR: Resumen semanal — ${items.length} oportunidades`,
      html
    })
  });
...
async function main() {
  ...
  const [sub, lic, pipeline] = await Promise.all([
    obtenerNovedadesSubvenciones(),
    obtenerNovedadesLicitaciones(),
    obtenerResumenPipeline()
  ]);
  const items = [...sub, ...lic];

  console.log(`📋 Encontradas ${items.length} oportunidades relevantes de la semana`);

  if (items.length === 0 && pipeline.presentadas === 0 && pipeline.ganadas === 0 && pipeline.perdidas === 0) {
    console.log('✅ Nada relevante esta semana, no se envía resumen.');
    process.exit(0);
  }

  try {
    await enviarEmail(items, pipeline);
    console.log(`✅ Resumen semanal enviado con ${items.length} oportunidades`);
  } catch (error) {
